...
class _GenContinue():
    def __new__(cls,nb_gens,type='gen'):
        class_ = utils.get_class("eoGenContinue"+TYPES[type])
        return class_(nb_gens)

class _EvalContinue():
    def __new__(cls,f,nb_evals,type='gen'):
        class_ = utils.get_class("eoEvalContinue"+TYPES[type])
        #if fun is callable...
        #if fun is evalFunc...
...
class _CombinedContinue():
    def __new__(cls,continue1,continue2=None,type='gen'):
        class_ = utils.get_class("eoCombinedContinue"+TYPES[type])
...
class _SteadyFitContinue():
    def __new__(cls,min_gens,steady_gens,type='gen'):
        class_ = utils.get_class("eoSteadyFitContinue"+TYPES[type])
        return class_(min_gens,steady_gens)

class _SecondsElapsedContinue():
    def __new__(cls,seconds,type='gen'):
        class_ = utils.get_class("eoSecondsElapsedContinue"+TYPES[type])
